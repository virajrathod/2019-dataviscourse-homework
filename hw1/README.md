...
Create horizontal bar charts for both the X and the Y dimensions of the data. Your bars should be start from the center. X and Y dimensions should extend to left and right respectively.

When you hover over a bar, its fill color should change.
...
Next, you should draw area charts of the same data. Area chart fo X dimension should be rotated to the left and for Y dimension to the right. An area chart is very much like the line chart (hint: you can probably re-use some of the code from before), but it is filled. See the following examples:

![Area Charts](figures/areas.png)

### Scatterplot

A scatterplot shows how two dimensions relate to each other. Plot the X dimension along the x-axis, and the Y dimension along the y-axis. When you hover over a circle, its fill color should change and x and y value of the particular point should be shown in a tooltip (hint: check out *title* element of SVG).
